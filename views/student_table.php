              ...
              if (mysqli_num_rows($result) > 0) 
              {
                while ($info = mysqli_fetch_assoc($result)) 
                {
                  ...
                  echo "<tr>" .
                    ...
                    if($_SESSION["account"] == 'root'){
                      echo "<td>
                        <button onclick=\"put_student('$name','$account','$email','$phone','$gender', '$department')\"class='call-btn btn btn-outline-primary btn-floating btn-sm ripple-surface' data-mdb-toggle='modal' data-mdb-target='#updateStudentModal'><i class='fa fa-pencil'></i></button>
                        <button onclick=\"put_student('$name','$account','$email','$phone','$gender', '$department')\"class='message-btn btn ms-2 btn-primary btn-floating btn-sm' data-mdb-toggle='modal' data-mdb-target='#deleteStudentModal'><i class='fa fa-trash'></i></button>
                      </td>";
                    }
                  echo "</tr>";
                }
...
<div class='modal fade' id='updateStudentModal' tabindex='-1' aria-labelledby='updateStudentModalLabel' aria-hidden='true'>
  ...
  <form method='post' action='./controller/user_controller.php'>
    ...
    <div class='modal-body'>
        ...
        <input value=3  required type='hidden' name='type' class='form-control' />
        <div class='form-outline mb-4'>
          <input id='department' required type='text' name='department' class='form-control' />
          <label class='form-label'>系所</label>
        </div>
      </div>
    </div>
...
<script>
function put_student(a, b, c, d, e, f){
  ...
  var elms = document.querySelectorAll("[id='department']");
  for(var i = 0; i < elms.length; i++) 
    elms[i].value=f
