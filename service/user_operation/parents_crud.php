<?php
    ...
    function parents_read_student($conn , $account){
        ...
        $sql = "SELECT * FROM user WHERE account IN (SELECT parent_account  FROM student JOIN parent ON parent.student_account = student.account WHERE student.account = ?)";        
        $stmt = $conn->prepare($sql);
    ...
    function parents_read_all($conn){
        ...
        $sql = "SELECT * FROM user JOIN parent ON user.account = parent.parent_account";
